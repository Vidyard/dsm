  ...
  class Invoker
    ...
    def run(&block)
      ...
      begin
        ...
        time.measure(@chunker.num_chunks_requested) do
          begin
        ...
        overall_avg_str = "Average Speed #{time.overall_average.round(3)}s"
        recent_avg_str = "Recent Average (10) Speed #{time.recent_average.round(3)}s"
        duration_str = "Runtime #{time.total_duration.round(3)}s"
        output.print "\r#{percent_str} | #{recent_avg_str} | #{overall_avg_str} | #{duration_str}"
